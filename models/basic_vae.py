...
# Groupnorm using 32 channels per group
...
# Upsample2x: upsample the input tensor by a factor of 2 and apply a 3x3 convolution with padding 1 stride 1
...
# Downsample2x: pad input matrix right and down with one column/row of zeros to account for odd shape?
# then convolve with 3x3 convolution with stride 2
...
# Groupnorm using 32 channels per group
# x -> GroupNorm -> SiLU -> Conv2d -> GroupNorm -> SiLU -> Dropout -> Conv2d = h
# h + x -> output
...
# Attention block
# qkv are treated as 3 groups of channels
# w_ratio ?
# Norm(x) -> conv to form qkv number of channels for efficiency
# extract q,k,v from qkv (use reshape and unbind)
# q: B,C,H,W -> B,HW,C
# k: B,C,H,W -> B,C,HW
# Perform batch matrix multiplication to get w: B,HW,HW; w[B,i,j]=sum_c q[B,i,C]k[B,C,j]
# Apply softmax to get w
# v: B,C,H,W -> B,C,HW
# !TODO: review attention mechanism
# w: permute dimensions such that bmm(v,w) -> h[B,i,j] = sum_i v[B,C,i] w[B,i,j]...
# reshape h to B,C,H,W and return x + proj_out(h) where proj_out is a 1x1 conv
        ...
        *,
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                block_in = block_out
                # add self attention block at the last block of the last level
